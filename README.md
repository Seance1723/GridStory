...
Without a configured catalog, the same header shows the current context read-only with a factual notice. Studio's initial scope comes from the `VITE_GRIDSTORY_ORGANIZATION`, `VITE_GRIDSTORY_TENANT`, `VITE_GRIDSTORY_WORKSPACE`, `VITE_GRIDSTORY_SITE`, `VITE_GRIDSTORY_ENVIRONMENT` and `VITE_GRIDSTORY_LOCALE` deployment values documented in `.env.example`. See [ADR 0030](docs/adr/0030-studio-capabilities-and-scope-selection.md) and [troubleshooting](docs/troubleshooting.md).
...
Delivery status: CMS-032, CMS-033 and CMS-034 completed the authorized navigation/context group; CMS-004 through CMS-007 are separately committed on the shared content/configuration branch. CMS-008 now exposes the existing application-owned design manifest as a lazy read-only Components catalog with component contracts and lifecycle evidence, tokens/breakpoints/variants, symbols/templates and guarded insertion through the existing composition editor. Its complete repository gate passes 690 active tests with 17 existing optional skips, and the final 66-scenario Chromium/Firefox/WebKit matrix passes. Detailed context and evidence is retained in ADRs 0030–0032, the design-system guide and the CMS administration gap analysis. No production readiness, external provider certification or deployment is claimed.

### Studio styles and preview

GridStory Studio imports one `apps/studio/src/styles/studio.scss` entry. It composes ordered Sass `@use` partials for foundation/tokens, management surfaces, authoring, collaboration, assets, design catalog, workflow/search, shell/navigation, calls to action, typography, forms, cards/spacing, states/themes, responsive layout, and accessibility. Shared native-control appearance belongs in `_form.scss`; button variants belong in `_cta.scss`; card surfaces and structural gaps belong in `_cards.scss`; readable text behavior belongs in `_typographic.scss`. Asset-only workspace layout belongs in `_assets.scss`, governed catalog layout in `_design.scss`; keep other feature-only layout in its owning feature partial and do not add a second global override layer.

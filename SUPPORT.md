...
Marketplace support metadata is a publisher-signed declaration, not a GridStory support entitlement. A `maintained` badge, policy URL, tested-runtime record, verified publisher, or passing automated review does not create a response-time guarantee or transfer responsibility for third-party package code. Operators own publisher due diligence, DNS/scanner adapter trust, scanner policy, package/evidence storage, capability grants, enablement, runtime isolation, and third-party incident coordination; see [Evidence-bound plugin marketplace](docs/marketplace.md).

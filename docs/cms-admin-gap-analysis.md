...
- Status: GOV-006 analysis complete; the inventories below retain their planning baseline. Completed delivery checkpoints run through section 29. Section 30 records the documentation-only CMS-011 visitor-menu proposal; its runtime remains unimplemented pending explicit T2 approval.
...
## 30. CMS-011 visitor navigation planning checkpoint

- Proposed: Codex, 2026-08-31, baseline `0b69c37` on `codex/website-management-gaps`. [ADR 0034](adr/0034-versioned-visitor-navigation-menus.md) is documentation-only; no menu schema, service, endpoint, Studio leaf, Vite output or test behavior has shipped.
...
- Planning evidence: `pnpm lint` passes all repository lint/boundary/ledger/security/tenant/readiness checks across 364 files, `pnpm format:check` passes across 361 files, and whitespace, exact-five-file and 70-local-link audits pass. No runtime/test/generated file changed and no defect was found; full implementation/browser/database/provider evidence is not claimed.
- Handoff: verify and commit the five-file planning fence (including ADR 0034) separately, then request explicit approval. Approval starts CMS-011 only; CMS-012 through CMS-015 retain separate plans, gates and verified commits on the same shared branch. No merge, push or deployment is included.

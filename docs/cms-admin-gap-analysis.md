...
## 25. CMS-006 factual editorial Home proposal

- Proposed: Codex, 2026-08-29, baseline `519235f` on `codex/content-configuration-foundations`. The T2 decision and exact runtime/test fence are in [ADR 0032](adr/0032-factual-editorial-home.md); explicit approval is required after the separate planning commit.
...
- Proposed minimal boundary: one strict versioned private/no-store editorial overview. Content is exact for declared `all-registered` or `pages-only` coverage with five recent items; eligible reviews are server-filtered by readable type, current workflow definition, existing review permission, role and separation of duties; releases and operator attention are minimized; every widget is independently `available`, `unavailable` or `error` with explicit total/display/limit/truncation evidence.
- Studio adds Home as a top-level finite destination. Recent/review links reuse exact Page/Collection history; release/operations links reuse existing screens; quick create delegates to CMS-004's canonical candidate/preflight/create path. Existing neutral colors and global controls remain. No chart, trend, greeting personalization, dashboard/plugin framework, analytics collection, task/notification model, new permission, storage schema, aggregate table, dependency or cross-tenant metric is proposed.
...
- Planned evidence: strict schema/core scope/minimization/filter/error tests; production API auth/no-store/no-input/non-enumeration tests; client scope validation; Home capability/history/loading/empty/error/retry/quick-create tests; and the complete repository plus Chromium/Firefox/WebKit six-width, keyboard, zoom, forced-color and WCAG matrix. No database/provider/recovery/deployment result is claimed by this documentation plan.
- Handoff: verify and commit the ADR 0032 planning checkpoint separately, then wait for explicit CMS-006 approval. CMS-007–010 retain their own delivery boundaries; no merge, push or deployment is included.

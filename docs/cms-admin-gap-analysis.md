...
## 29. CMS-010 safe configuration inventory proposal

- Proposed: Codex, 2026-08-29, baseline `ec408a6`. [ADR 0033](adr/0033-safe-configuration-inventory.md) is a documentation-only T2 checkpoint; runtime and test files remain unchanged pending explicit approval.
...
- Decision: add one strict private/no-store inventory with independently authorized locales/environments, models/routes and media policy/provider-availability sections. Reuse only existing `locales.read`, `schema.read` and `asset.read`; mark every item `code`, `operator` or `editor` owned plus read-only; add a finite Settings group/Configuration leaf; return no generic key/value metadata or edit control.
...
- Planned evidence: strict schema/core scope/minimization tests; API auth/no-store/no-input/independent-decision and forbidden-material tests; client scope validation; full/schema/asset/locale/denied Studio cases; zero configuration mutation requests; complete navigation/accessibility/six-width three-engine and repository gates. No database, provider execution or deployment certification is claimed.
- Handoff: verify and commit this plan separately, then wait for explicit ADR 0033 approval. Approval starts CMS-010 only; later CMS tasks retain their own gates.

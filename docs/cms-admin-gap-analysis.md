...
The original first recommendation was **CMS-001: reorganize only the 19 existing destinations** using the mapping above. CMS-001 and **CMS-002: stable Studio locations, deep links and history-safe navigation** are implemented and verified. AUTH-001 subsequently closed the verified authentication prerequisite. The user approved [ADR 0030](adr/0030-studio-capabilities-and-scope-selection.md)'s sequential CMS-032 → CMS-033 → CMS-034 delivery. CMS-032's API/client foundation is implemented but **blocked by the required WebKit history gate**. The next action is **approve its bounded three-file Studio/test repair amendment for BUG-0441/0442, then repair and verify before closing CMS-032**. CMS-033/034 have not started and parent CMS-003 is incomplete; a WIP checkpoint is not shipped completion.

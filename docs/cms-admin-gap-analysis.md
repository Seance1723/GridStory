...
- Status: GOV-006 analysis complete; the inventories below retain their planning baseline. Completed delivery checkpoints run through section 29. Section 30 records the approved CMS-011 visitor-menu implementation and its bounded delivery evidence.
...
- **CMS-011 contract checkpoint:** [ADR 0034](adr/0034-versioned-visitor-navigation-menus.md) proposes an immutable 1–64-character menu key, localized arrays limited to 100 items and depth 3, stable parent-before-child IDs, exactly one routed relation or credential-free absolute HTTP(S) URL, deterministic default-locale identity, lifecycle validation across draft/published/release future views, cache tags for menu and route targets, and strict private/public projections. Existing update rollback is retained; first-publication releases remain explicitly non-rollbackable because this task adds no unpublish semantics.
...
## 30. CMS-011 versioned visitor navigation

- Proposed: Codex, 2026-08-31, baseline `0b69c37` on `codex/website-management-gaps`; planning commit `9f9c003`. The user explicitly approved [ADR 0034](adr/0034-versioned-visitor-navigation-menus.md) through `proceed`; CMS-012 through CMS-015 remain outside that approval.
- Inward decision: reuse canonical content/relation contracts, immutable revisions, localization, optimistic updates, workflows, future-state releases, audit/outbox, regional published readers and scoped cache tags. Add one reserved type and one trusted code-owned lifecycle validator/resolver; add no repository, second revision store, permission action or generic runtime navigation framework.
- Representation: one immutable stable menu key, editor name and localized ordered flat item list. Stable IDs and earlier `parentId` values form a maximum-depth-3 tree within 100 items. Each item has exactly one current-route internal reference or credential-free absolute HTTP(S) URL. The public response is published-only and minimized; applications own semantic markup, interaction, placement, router use and CSS.
- Release decision: a publication view overlays one candidate on current published state; a release view overlays every pinned candidate, permitting a new page/menu pair to validate together. Existing rollback restores previous published pointers for updates. A first-publication release has no previous pointer and remains explicitly non-rollbackable; corrective publication is the recovery path until CMS-023 defines retirement semantics.
- Outward saturation: official WordPress, Sanity and Contentful guidance converges on named ordered menus, structured internal/external links and optional hierarchy. WAI guidance keeps ordinary site navigation in labelled semantic navigation/list/link markup rather than an application-menu widget. GridStory adopts those data/workflow boundaries, not vendor theme controls, mega-menu schemas or runtime code.
- Delivered: one reserved canonical schema and generated example type, trusted lifecycle validator/service, stable-key management creation, private draft preview, anonymous published locale-aware delivery, exact menu/target dependency tags, default Header/Footer seeds, universal-client methods, independently authorized **Navigation > Menus**, and semantic application-owned Vite Header/Footer rendering. Existing repositories, content/workflow permissions, revisions, localization, regional reads, releases, audit and outbox remain authoritative; no second store, permission, dependency, router/theme takeover, unpublish/delete behavior or provider was added.
- Verification findings: BUG-0553–BUG-0576 permanently retain generator/build order, exact-optional typing, worker permission, stable-key conflict, fixture, initial destination, workflow-label, browser-cache, async selection, contrast and bounded cross-engine findings. Every CMS-011 finding is resolved; the retained WebKit/Firefox one-offs are non-reproducing under exact no-retry repetition plus clean full matrices, with no assertion suppression or global retry.
- Final evidence: `pnpm check` passes 738 active tests with 17 existing optional skips (97 schema, 42 client, 175 core, 5 React, 107 API and 312 Studio), all generated/boundary/ledger/security/tenant/readiness/type gates, React 18.3.1 SSR and production builds. The final no-retry browser matrix passes 78/78 (26 Chromium, 26 Firefox, 26 WebKit), including real draft/public isolation, distinct approval, publication/revalidation, semantic application output, all-destination six-width containment and unsuppressed WCAG 2.2 A/AA.
- Handoff: CMS-011 is complete and receives its own verified implementation commit on the shared branch. CMS-012 is the next separate planning/approval checkpoint; CMS-013 through CMS-015 retain their dependencies, gates and verified commits. No merge, push, deployment, PostgreSQL/recovery or provider certification is included.

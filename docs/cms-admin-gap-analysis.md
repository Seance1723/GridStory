...
The original first recommendation was **CMS-001: reorganize only the 19 existing destinations** using the mapping above. CMS-001 and **CMS-002: stable Studio locations, deep links and history-safe navigation** are implemented and verified. CMS-003 inspection then exposed BUG-0433; the separately approved **AUTH-001** fix is now implemented and verified in [ADR 0029](adr/0029-production-preview-authentication-boundary.md). The next action is **approve the scoped CMS-003 plan in [ADR 0030](adr/0030-studio-capabilities-and-scope-selection.md), then implement CMS-032 alone**. CMS-003 now has three sequential delivery units (CMS-032 → CMS-033 → CMS-034); the broader program does not approve their contracts automatically.

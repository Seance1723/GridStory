...
Authentication, credential lifecycle, and break-glass events become applicable when M6-002 adds production identity/session operations. Until then, the development-header identity is not a production authentication system. Failed authorization is already exported as a bounded `forbidden` request error type, and privileged mutations/publication remain authoritatively represented in the audit chain plus correlated route/method telemetry. Plugin and external-adapter failures are represented by bounded error types and domain outcomes; plugin code, payloads, credentials, and draft content are excluded.

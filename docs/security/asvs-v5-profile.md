...
`GS-SEC-040` and `THREAT-0036` link strict federation schemas/limits; complete-scope optimistic memory, SQLite, and qualified PostgreSQL state; exact deployed-type fingerprints; injected Ed25519 signing; request/time/instance/scope/offer/type/revision/checksum/attribution/checkpoint validation; configured no-redirect same-origin bounded HTTPS; distinct private read/manage/consume/sync authorization; signed published-only source routes; live no-retention reads; expiring digest-reviewed source-revalidated atomic idempotent mirrors; sequence blockers and withdrawal tombstones; minimized no-store public delivery; typed REST/client/neutral Studio controls; PostgreSQL persistence/logical restore; and live SQLite recovery.

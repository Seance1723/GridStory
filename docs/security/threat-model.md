...
| THREAT-0002 | Development identity exposed in production | S/E | 15 High | Production mode rejects actor/role/principal headers, backend-verifies opaque sessions, tenant-binds SCIM credentials, matches credential dispatch to the resolved route/method and rejects unbound private context. AUTH-001 covers preview-prefix and activation/revocation confusion; trusted-proxy/TLS and live IdP deployment conformance remain explicit beta evidence. |
...
| THREAT-0022 | Sensitive error, debug, or effective-configuration disclosure | I | 12 High | Generic responses remain enforced; M5-004 removes detailed public readiness drift and raw telemetry material. CMS-010 adds a strict parameter-free private/no-store inventory over closed safe inputs, permission-filtered context choices, independent existing permissions and fixed provider modes, with serialized forbidden-material regressions. Production debug-off and deployment configuration remain external validation. |
...
## M8-004 generated interoperability and pull-only fleet boundary

`THREAT-0038` covers schema drift, public metadata overexposure, preview annotation leakage, fleet scope bleed, caller-selected SSRF, redirect/origin confusion, oversized or malformed evidence, instance/service/specification substitution, stale condition claims, raw diagnostic retention, and accidental remote-control expansion. The public surface contains only configured instance/service/protocol identity and four generated version/digest/schema-link descriptors. Tenant schemas, component inventory, content, drafts, credentials, adapter configuration, fleet members, and live status remain private.

Fleet observations are complete-scope, separately authorized, bounded, generation-bound, expiring, and recoverable. The maintained observer accepts only a deployment-controlled credential-free HTTPS origin, makes exact GET-only requests with redirects disabled, validates response origin, and passes strict bounded evidence to core. Core independently validates freshness and all required specification digests, records generic finite conditions, and projects expired evidence as Unknown. Browser callers provide only a configured adapter ID; they cannot provide a URL or credential. GridStory exposes no discovery scan, inbound enrollment, agent, heartbeat, scheduler, secret, shell, remote write, provision, deploy, upgrade, rollback, traffic, content, workflow, release, or publication operation. Deployment egress/private-address/DNS-rebinding/TLS controls remain operator evidence.

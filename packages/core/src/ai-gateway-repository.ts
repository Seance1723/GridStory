...
export function emptyAiGatewayDocument(
  scope: ContentScope,
...
export interface AiGatewayRepository {
  get(scope: ContentScope): Awaitable<AiGatewayDocument | null>;
  save(document: AiGatewayDocument, expectedVersion: number | null): Awaitable<void>;
  close(): Awaitable<void>;
}

export class InMemoryAiGatewayRepository implements AiGatewayRepository {
  readonly #documents = new Map<string, AiGatewayDocument>();

  get(scope: ContentScope): AiGatewayDocument | null {
    const document = this.#documents.get(contentScopeKey(scope));
    return document ? structuredClone(document) : null;
  }

  save(document: AiGatewayDocument, expectedVersion: number | null): void {
    const parsed = aiGatewayDocumentSchema.parse(document);
...
export class SqliteAiGatewayRepository implements AiGatewayRepository {
  readonly #database: DatabaseSync;

  constructor(options: { filename: string }) {
    this.#database = new DatabaseSync(options.filename);
    this.#database.exec(`
      CREATE TABLE IF NOT EXISTS gridstory_ai_gateway_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_gridstory_ai_gateway_updated
        ON gridstory_ai_gateway_documents (
  ...
  get(scope: ContentScope): AiGatewayDocument | null {
    const row = this.#database
      .prepare('SELECT payload FROM gridstory_ai_gateway_documents WHERE scope_key = ?')
      .get(contentScopeKey(scope)) as unknown as PayloadRow | undefined;
    return row ? aiGatewayDocumentSchema.parse(JSON.parse(row.payload)) : null;
  }
  ...
  save(document: AiGatewayDocument, expectedVersion: number | null): void {
    ...
    if (expectedVersion === null) {
      try {
        this.#database
          .prepare(
            `INSERT INTO gridstory_ai_gateway_documents (
    ...
    const result = this.#database
      .prepare(
        `UPDATE gridstory_ai_gateway_documents
...
export class PostgresAiGatewayRepository implements AiGatewayRepository {
  ...
  constructor(options: { connectionString?: string; pool?: Pool; schema?: string }) {
    ...
    if (!/^[a-z_][a-z0-9_]*$/iu.test(schema)) {
      throw new Error('PostgreSQL AI gateway schema name is invalid.');
    }
    if (!options.pool && !options.connectionString) {
      throw new Error('PostgreSQL AI gateway connectionString or pool is required.');
    }
    this.#pool =
      options.pool ?? new Pool({ connectionString: options.connectionString } as PoolConfig);
    this.#ownsPool = !options.pool;
    const quotedSchema = `"${schema}"`;
    this.#table = `${quotedSchema}.gridstory_ai_gateway_documents`;
  ...
  async get(scope: ContentScope): Promise<AiGatewayDocument | null> {
    await this.#ready;
    const result = await this.#pool.query<{ payload: unknown }>(
      `SELECT payload FROM ${this.#table} WHERE scope_key = $1`,
      [contentScopeKey(scope)],
    );
    return result.rows[0] ? aiGatewayDocumentSchema.parse(result.rows[0].payload) : null;
  }

  async save(document: AiGatewayDocument, expectedVersion: number | null): Promise<void> {
    await this.#ready;
    const parsed = aiGatewayDocumentSchema.parse(document);
    if (expectedVersion === null) {

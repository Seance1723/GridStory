...
export function emptyKnowledgeDocument(
  scope: ContentScope,
...
export interface KnowledgeRepository {
  get(scope: ContentScope): Awaitable<KnowledgeDocument | null>;
  save(document: KnowledgeDocument, expectedVersion: number | null): Awaitable<void>;
...
export class InMemoryKnowledgeRepository implements KnowledgeRepository {
  ...
  get(scope: ContentScope): KnowledgeDocument | null {
    const document = this.#documents.get(contentScopeKey(scope));
    return document ? structuredClone(document) : null;
  }

  save(document: KnowledgeDocument, expectedVersion: number | null): void {
...
export class SqliteKnowledgeRepository implements KnowledgeRepository {
  ...
  constructor(options: { filename: string }) {
    ...
    this.#database.exec(`
      CREATE TABLE IF NOT EXISTS gridstory_knowledge_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_gridstory_knowledge_updated
        ON gridstory_knowledge_documents (
  ...
  get(scope: ContentScope): KnowledgeDocument | null {
    const row = this.#database
      .prepare('SELECT payload FROM gridstory_knowledge_documents WHERE scope_key = ?')
      .get(contentScopeKey(scope)) as unknown as PayloadRow | undefined;
    return row ? knowledgeDocumentSchema.parse(JSON.parse(row.payload)) : null;
  }
  ...
  save(document: KnowledgeDocument, expectedVersion: number | null): void {
    ...
    if (expectedVersion === null) {
      try {
        this.#database
          .prepare(
            `INSERT INTO gridstory_knowledge_documents (
    ...
    const result = this.#database
      .prepare(
        `UPDATE gridstory_knowledge_documents
...
export class PostgresKnowledgeRepository implements KnowledgeRepository {
  ...
  constructor(options: { connectionString?: string; pool?: Pool; schema?: string }) {
    ...
    if (!/^[a-z_][a-z0-9_]*$/iu.test(schema)) {
      throw new Error('PostgreSQL knowledge schema name is invalid.');
    }
    if (!options.pool && !options.connectionString) {
      throw new Error('PostgreSQL knowledge connectionString or pool is required.');
    }
    this.#pool =
      options.pool ?? new Pool({ connectionString: options.connectionString } as PoolConfig);
    this.#ownsPool = !options.pool;
    const quotedSchema = `"${schema}"`;
    this.#table = `${quotedSchema}.gridstory_knowledge_documents`;
  ...
  async get(scope: ContentScope): Promise<KnowledgeDocument | null> {
    await this.#ready;
    const result = await this.#pool.query<{ payload: unknown }>(
      `SELECT payload FROM ${this.#table} WHERE scope_key = $1`,
      [contentScopeKey(scope)],
    );
    return result.rows[0] ? knowledgeDocumentSchema.parse(result.rows[0].payload) : null;
  }

  async save(document: KnowledgeDocument, expectedVersion: number | null): Promise<void> {
    await this.#ready;
    const parsed = knowledgeDocumentSchema.parse(document);
    if (expectedVersion === null) {

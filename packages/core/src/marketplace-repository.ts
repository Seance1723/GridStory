...
export type MarketplaceDocument = MarketplaceSnapshot;

export function emptyMarketplaceDocument(
  scope: ContentScope,
...
function writeConflict(): GridStoryError {
  return new GridStoryError(
    'Marketplace state changed during this operation.',
    'marketplace_write_conflict',
    409,
  );
}

export interface MarketplaceRepository {
  get(scope: ContentScope): Awaitable<MarketplaceDocument | null>;
  save(document: MarketplaceDocument, expectedVersion: number | null): Awaitable<void>;
...
export class InMemoryMarketplaceRepository implements MarketplaceRepository {
  readonly #documents = new Map<string, MarketplaceDocument>();

  get(scope: ContentScope): MarketplaceDocument | null {
    const document = this.#documents.get(contentScopeKey(scope));
    return document ? structuredClone(document) : null;
  }

  save(document: MarketplaceDocument, expectedVersion: number | null): void {
    const parsed = marketplaceSnapshotSchema.parse(document);
...
export class SqliteMarketplaceRepository implements MarketplaceRepository {
  readonly #database: DatabaseSync;

  constructor(options: { filename: string }) {
    this.#database = new DatabaseSync(options.filename);
    this.#database.exec(`
      CREATE TABLE IF NOT EXISTS gridstory_marketplace_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_gridstory_marketplace_updated
        ON gridstory_marketplace_documents (
  ...
  get(scope: ContentScope): MarketplaceDocument | null {
    const row = this.#database
      .prepare('SELECT payload FROM gridstory_marketplace_documents WHERE scope_key = ?')
      .get(contentScopeKey(scope)) as unknown as PayloadRow | undefined;
    return row ? marketplaceSnapshotSchema.parse(JSON.parse(row.payload)) : null;
  }
  ...
  save(document: MarketplaceDocument, expectedVersion: number | null): void {
    ...
    if (expectedVersion === null) {
      try {
        this.#database
          .prepare(
            `INSERT INTO gridstory_marketplace_documents (
    ...
    const result = this.#database
      .prepare(
        `UPDATE gridstory_marketplace_documents
    ...
    const rows = this.#database
      .prepare(
        `SELECT organization_id, tenant_id, workspace_id, site_id, environment_id, locale
         FROM gridstory_marketplace_documents
...
export class PostgresMarketplaceRepository implements MarketplaceRepository {
  ...
  constructor(options: { connectionString?: string; pool?: Pool; schema?: string }) {
    ...
    if (!/^[a-z_][a-z0-9_]*$/iu.test(schema)) {
      throw new Error('PostgreSQL marketplace schema name is invalid.');
    }
    if (!options.pool && !options.connectionString) {
      throw new Error('PostgreSQL marketplace connectionString or pool is required.');
    }
    this.#pool =
      options.pool ?? new Pool({ connectionString: options.connectionString } as PoolConfig);
    this.#ownsPool = !options.pool;
    const quotedSchema = `"${schema}"`;
    this.#table = `${quotedSchema}.gridstory_marketplace_documents`;
  ...
  async get(scope: ContentScope): Promise<MarketplaceDocument | null> {
    await this.#ready;
    const result = await this.#pool.query<{ payload: unknown }>(
      `SELECT payload FROM ${this.#table} WHERE scope_key = $1`,
      [contentScopeKey(scope)],
    );
    return result.rows[0] ? marketplaceSnapshotSchema.parse(result.rows[0].payload) : null;
  }

  async save(document: MarketplaceDocument, expectedVersion: number | null): Promise<void> {
    await this.#ready;
    const parsed = marketplaceSnapshotSchema.parse(document);
    if (expectedVersion === null) {

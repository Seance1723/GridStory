...
export type MigrationDocument = MigrationSnapshot;

export function emptyMigrationDocument(
  scope: ContentScope,
...
function writeConflict(): GridStoryError {
  return new GridStoryError(
    'Migration state changed during this operation.',
    'migration_write_conflict',
    409,
  );
}

export interface MigrationRepository {
  get(scope: ContentScope): Awaitable<MigrationDocument | null>;
  save(document: MigrationDocument, expectedVersion: number | null): Awaitable<void>;
...
export class InMemoryMigrationRepository implements MigrationRepository {
  readonly #documents = new Map<string, MigrationDocument>();

  get(scope: ContentScope): MigrationDocument | null {
    const document = this.#documents.get(contentScopeKey(scope));
    return document ? structuredClone(document) : null;
  }

  save(document: MigrationDocument, expectedVersion: number | null): void {
    const parsed = migrationSnapshotSchema.parse(document);
...
export class SqliteMigrationRepository implements MigrationRepository {
  ...
  constructor(options: { filename: string }) {
    ...
    this.#database.exec(`
      CREATE TABLE IF NOT EXISTS gridstory_migration_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_gridstory_migration_updated
        ON gridstory_migration_documents (
  ...
  get(scope: ContentScope): MigrationDocument | null {
    const row = this.#database
      .prepare('SELECT payload FROM gridstory_migration_documents WHERE scope_key = ?')
      .get(contentScopeKey(scope)) as unknown as PayloadRow | undefined;
    return row ? migrationSnapshotSchema.parse(JSON.parse(row.payload)) : null;
  }
  ...
  save(document: MigrationDocument, expectedVersion: number | null): void {
    ...
    if (expectedVersion === null) {
      try {
        this.#database
          .prepare(
            `INSERT INTO gridstory_migration_documents (
    ...
    const result = this.#database
      .prepare(
        `UPDATE gridstory_migration_documents
    ...
    const rows = this.#database
      .prepare(
        `SELECT organization_id, tenant_id, workspace_id, site_id, environment_id, locale
         FROM gridstory_migration_documents
...
export class PostgresMigrationRepository implements MigrationRepository {
  ...
  constructor(options: { connectionString?: string; pool?: Pool; schema?: string }) {
    ...
    if (!/^[a-z_][a-z0-9_]*$/iu.test(schema)) {
      throw new Error('PostgreSQL migration schema name is invalid.');
    }
    if (!options.pool && !options.connectionString) {
      throw new Error('PostgreSQL migration connectionString or pool is required.');
    }
    this.#pool =
      options.pool ?? new Pool({ connectionString: options.connectionString } as PoolConfig);
    this.#ownsPool = !options.pool;
    const quotedSchema = `"${schema}"`;
    this.#table = `${quotedSchema}.gridstory_migration_documents`;
  ...
  async get(scope: ContentScope): Promise<MigrationDocument | null> {
    await this.#ready;
    const result = await this.#pool.query<{ payload: unknown }>(
      `SELECT payload FROM ${this.#table} WHERE scope_key = $1`,
      [contentScopeKey(scope)],
    );
    return result.rows[0] ? migrationSnapshotSchema.parse(result.rows[0].payload) : null;
  }

  async save(document: MigrationDocument, expectedVersion: number | null): Promise<void> {
    await this.#ready;
    const parsed = migrationSnapshotSchema.parse(document);
    if (expectedVersion === null) {

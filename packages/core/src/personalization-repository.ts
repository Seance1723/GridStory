...
export type PersonalizationDocument = PersonalizationSnapshot;

export function emptyPersonalizationDocument(
  scope: ContentScope,
  actorId = 'system',
  ...
  return {
    ...
    draft: {
      revision: 1,
      configuration: { purposes: [], attributes: [], audiences: [], decisions: [] },
      updatedAt: timestamp,
      updatedBy: actorId,
    },
    experiments: [],
...
function writeConflict(): GridStoryError {
  return new GridStoryError(
    'Personalization state changed during this operation.',
    'personalization_write_conflict',
    409,
  );
}

export interface PersonalizationRepository {
  get(scope: ContentScope): Awaitable<PersonalizationDocument | null>;
  save(document: PersonalizationDocument, expectedVersion: number | null): Awaitable<void>;
...
export class InMemoryPersonalizationRepository implements PersonalizationRepository {
  readonly #documents = new Map<string, PersonalizationDocument>();

  get(scope: ContentScope): PersonalizationDocument | null {
    const document = this.#documents.get(contentScopeKey(scope));
    return document ? structuredClone(document) : null;
  }

  save(document: PersonalizationDocument, expectedVersion: number | null): void {
    const parsed = personalizationSnapshotSchema.parse(document);
...
export class SqlitePersonalizationRepository implements PersonalizationRepository {
  ...
  constructor(options: { filename: string }) {
    ...
    this.#database.exec(`
      CREATE TABLE IF NOT EXISTS gridstory_personalization_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_gridstory_personalization_updated
        ON gridstory_personalization_documents (
  ...
  get(scope: ContentScope): PersonalizationDocument | null {
    const row = this.#database
      .prepare('SELECT payload FROM gridstory_personalization_documents WHERE scope_key = ?')
      .get(contentScopeKey(scope)) as unknown as PayloadRow | undefined;
    return row ? personalizationSnapshotSchema.parse(JSON.parse(row.payload)) : null;
  }
  ...
  save(document: PersonalizationDocument, expectedVersion: number | null): void {
    ...
    if (expectedVersion === null) {
      try {
        this.#database
          .prepare(
            `INSERT INTO gridstory_personalization_documents (
    ...
    const result = this.#database
      .prepare(
        `UPDATE gridstory_personalization_documents
    ...
    const rows = this.#database
      .prepare(
        `SELECT organization_id, tenant_id, workspace_id, site_id, environment_id, locale
         FROM gridstory_personalization_documents
...
export class PostgresPersonalizationRepository implements PersonalizationRepository {
  readonly #pool: Pool;
  ...
  constructor(options: { connectionString?: string; pool?: Pool; schema?: string }) {
    ...
    if (!/^[a-z_][a-z0-9_]*$/iu.test(schema)) {
      throw new Error('PostgreSQL personalization schema name is invalid.');
    }
    if (!options.pool && !options.connectionString) {
      throw new Error('PostgreSQL personalization connectionString or pool is required.');
    }
    this.#pool =
      options.pool ?? new Pool({ connectionString: options.connectionString } as PoolConfig);
    this.#ownsPool = !options.pool;
    const quotedSchema = `"${schema}"`;
    this.#table = `${quotedSchema}.gridstory_personalization_documents`;
  ...
  async get(scope: ContentScope): Promise<PersonalizationDocument | null> {
    await this.#ready;
    const result = await this.#pool.query<{ payload: unknown }>(
      `SELECT payload FROM ${this.#table} WHERE scope_key = $1`,
      [contentScopeKey(scope)],
    );
    return result.rows[0] ? personalizationSnapshotSchema.parse(result.rows[0].payload) : null;
  }

  async save(document: PersonalizationDocument, expectedVersion: number | null): Promise<void> {
    await this.#ready;
    const parsed = personalizationSnapshotSchema.parse(document);
    if (expectedVersion === null) {

import type { AssetReference, RichTextDocument } from './authoring.js';
import type {
  ComponentManifest,
  ComponentNode,
  ContentReference,
...
type FieldValue<Field, Objects = never> = Field extends { type: 'component-tree' }
  ? ComponentNode[]
  : Field extends { type: 'rich-text' }
    ? RichTextDocument
    : Field extends { type: 'asset' }
      ? AssetReference
...
function fieldType(field: FieldDefinition, objectNames: Map<string, string>): string {
  ...
  if (field.type === 'rich-text') return 'RichTextDocument';
  if (field.type === 'asset') return 'AssetReference';
...
export function generateTypeScriptContracts(
  ...
  const imports = [
    ...
    ...(schemas.some((schema) => schema.fields.some((field) => field.type === 'rich-text'))
      ? ['RichTextDocument']
      : []),
    ...(schemas.some((schema) => schema.fields.some((field) => field.type === 'asset'))
      ? ['AssetReference']
      : []),
  ];
  ...
  if (imports.length > 3) {
    lines.push(
      ...
      `} from ${stringLiteral(options.schemaImport ?? '@gridstory/schema')};`,
    );
